...
static void ChooseFont(void)
{
    ...
    int e = SDL_GetCurrentDisplayMode(0, &info); // 0 is used because we only need the main display
    if (e != 0)
    {
...
#else
    else if (info.w >= 1920 && info.h >= 1080)
    {
        font = &large_font;
    }
#endif
...
int TXT_Init(void)
{
    ...
    // FIXME : Window title is hardhacked to 'textscreen' for now
    window = SDL_CreateWindow(
			      "textscreen",                      // window title
			      SDL_WINDOWPOS_UNDEFINED,           // initial x position
			      SDL_WINDOWPOS_UNDEFINED,           // initial y position
			      TXT_SCREEN_W * font->w,            // width, in pixels
			      TXT_SCREEN_H * font->h,            // height, in pixels
			      0                                  // flags
			      );
    ...
    // draw everything into an intermediate 8-bit surface
